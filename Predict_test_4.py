...
def thread_judge(mir_q):
    ...
    for _ in cycle((None,)):
        ...
        if i == 2:
            # Calculate the distance and speed of tag
            distance_1 = (int(last_tag_x) - int(tag_x))**2 + (int(last_tag_y) - int(tag_y))**2
            distance_tag = math.sqrt(distance_1)
            time = 0.1
            speed = (distance_tag / 10) / time
            print('Tag moving distance =', distance_tag, 'Tag speed =', speed, 'm/s')

            # Calculate the moving direction of tag
...
def main():
    ...
    # follow_thread = threading.Thread(target=thread_follow)
